...
test.describe(
  "Warehouse GraphQL API — trip load",
  { tag: ["@api", "@warehouse", "@trip-load"] },
  () => {
    test.describe.configure({ timeout: 180_000 });

    test(
      "create trip load then delete it",
      { tag: ["@trip-load", "@delete", "@warehouse-regression"] },
      async ({ api }) => {
    ...
    test(
      ...
      { tag: ["@trip-load", "@scales", "@warehouse-regression"] },
      async ({ api }) => {
        const { tripId, channelType } = tripLoadEnv();
        test.skip(!tripId, "Set WAREHOUSE_TRIP_ID to a valid trip id");
    ...
    test(
      ...
      {
        tag: ["@trip-load", "@quality", "@create-trip-load-with-quality", "@warehouse-regression"],
      },
      async ({ api }) => {
        const { tripId, channelType } = tripLoadEnv();
        test.skip(!tripId, "Set WAREHOUSE_TRIP_ID to a valid trip id");
